...
class EvaHTTPClient:
    ...
    Eva HTTP client
	 ...
	 - api_token_id (string):           The ID of the valid API token, also retrievable from the Choreograph config page
    ...
    """
    def __init__(self, host_ip, api_token, api_token_id, custom_logger=None, request_timeout=5, renew_period=60*20):
    ...
    def auth_create_session(self):
        ...
        r = self.__api_request('POST', 'auth', payload=json.dumps({'token': self.api_token, 'token_id': self.api_token_id}), headers={})
